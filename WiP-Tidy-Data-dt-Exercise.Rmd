...
## Confirming "Infdicator.Name" and "Indicator.Code" have the same values for all observations
...
## Reshaping to long format & Removing the missing data
...
## Time trends for Greece

```{r}
GR<- WP[Country %in% "Greece"]
GR
```


## Visualisation for PT
...
WP[Country %in% "Portugal"] %>%
  ...
  ylab("% Women in Parliament PT")
```


## Visualisation for GR
...
GR<- WP[Country %in% "Greece"] %>%
  ...
  geom_line() + geom_point() +
  scale_y_continuous(limits = c(0, 50)) +
  ylab("% Women in Parliament GR")
GR
```

## PT, GR vs other EU countries

```{r}
WP[Country %in% c("Portugal", "Greece", "Sweden", "Spain",
                  "Hingary", "Romania", "Finland", "Germnay",
                  "European Union")] %>%
...
# Countrues with the % of women in parliament

```{r}
WP[order(-pctWiP), head(.SD, 10)]
```

# Countries with % by year
